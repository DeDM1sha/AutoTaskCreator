...
static void SetProgramFile_Unicode (const std::string TypeUnicode) {
    ...
    } // установка кириллицы дл€ программы
    ...
    } // установка кириллицы дл€ работы с файлом

} // функци€ дл€ смены кодировки в зависимости от задачи
...
class Class_BanLists : public AbstractClass_ConfigEditor {
    ...
    private:
		...
		std::string SaveTag_ClientsList; // “эг-сохранени€ клиентов в бан-листе
		std::string SaveTag_WorkersList; // “эг-сохранени€ исполнителей в бан-листе

    public:

        Class_BanLists (const Class_Settings& Settings) {

                if (Check_BanlistsFile (Settings))
                    Load_LocalBanlists (Settings);
    ...
    //////////////////////////////////////////////
        ...
        } // метод обнулени€ бан-листов

        const bool Check_BanlistsFile (const Class_Settings& Settings);
        const void Load_LocalBanlists (const Class_Settings& Settings);
        const void Update_Banlists (const Class_Settings&);
        const bool Check_Client_inBanlists (const Class_Settings& Settings, std::string&) const;

}; // класс бан-листа

const bool Class_BanLists::Check_BanlistsFile (const Class_Settings& Settings) {

    bool Existence = true; // существование локального списка бан-листа
    bool Clients_Founded = false;
    bool Workers_Founded = false;
    ConfigFile_Open (Settings.getBanlist_Path ());

    std::ifstream Read (Settings.getBanlist_Path ().c_str ());
    ...
    Read.close ();
        ...
        if (!Existence || (!Clients_Founded || !Workers_Founded)) {
            ...
            remove (Settings.getBanlist_Path ().c_str ());

            std::ofstream Write (Settings.getBanlist_Path ().c_str ());
    ...
    std::ifstream Check (Settings.getBanlist_Path ().c_str ());
    ...
    ConfigFile_Close (Settings.getBanlist_Path ());

    return Existence;

} // метод проверки существовани€ файла с конфигом

const void Class_BanLists::Load_LocalBanlists (const Class_Settings& Settings) {
...
const void Class_BanLists::Update_Banlists (const Class_Settings& Settings) {
	...
	//std::string Parser_Clients_Command = "start modules\\parser_GnuWin32\\bin\\wget.exe " + Settings.getUrl_BanList_Clients () + " --no-check-certificate Чoutput-document=\"index.html\"";
	std::string Str = "\0";
	...
	Clear_Banlists ();
        ...
        } // ожидание полного скачивани€ всего файла
...
const bool Class_BanLists::Check_Client_inBanlists (const Class_Settings& Settings, std::string& Str) const {
